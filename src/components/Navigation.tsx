...
export default function Navigation() {
  ...
  return (
    <div
      ref={navRef}
      className={`fixed top-0 left-0 w-full z-50 bg-white transition-all duration-300 ease-in-out ${expandedMenu ? "shadow-xl" : "shadow-sm"
        }`}
            ...
            <ul className="flex space-x-8">
              {Object.entries(navItems).map(([id, item]) => (
                <li key={id} className="relative nav-item" onMouseEnter={() => handleMouseEnter(id)}>
                  <Link
                    href={item.href}
                    className={`hover:text-[#2563EB] px-3 py-2 rounded-md transition duration-300 ease-in-out hover:bg-gray-50 ${expandedMenu === id ? "text-[#2563EB]" : ""
                      }`}
                  >
                    {item.name}
                  </Link>
